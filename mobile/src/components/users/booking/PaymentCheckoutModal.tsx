...
const checkoutHtml = (order: PaymentOrder, booking: Booking) => {
  const options = {
    key: order.keyId,
    amount: order.amountInPaise,
    currency: order.currency,
    name: "Barati Gharati",
    description: `${order.advancePercentage ? `${order.advancePercentage}% advance` : "Remaining payment"} for ${booking.vendorName}`,
...
export function PaymentCheckoutModal({ booking, mode, visible, onClose, onPaid }: Props) {
  ...
  const prepare = async () => {
    ...
    setOrder(null);
    try {
      setOrder(mode === "advance" ? await createAdvanceOrder(booking.id) : await createRemainingOrder(booking.id));
    } catch (reason) {
  ...
  useEffect(() => {
    if (visible && booking) void prepare();
    if (!visible) {
      setOrder(null);
      setError(null);
      setVerifying(false);
    }
  }, [visible, booking?.id, mode]);

  const html = useMemo(() => order && booking ? checkoutHtml(order, booking) : "", [order, booking]);

  const handleMessage = async (event: WebViewMessageEvent) => {
    if (!booking) return;
    try {
      const message = JSON.parse(event.nativeEvent.data) as { type: string; data?: RazorpaySuccess; message?: string };
      ...
      setVerifying(true);
      setError(null);
      if (mode === "advance") await verifyAdvancePayment(booking.id, message.data);
      else await verifyRemainingPayment(booking.id, message.data);
      await onPaid();
      setVerifying(false);
      onClose();
    } catch (reason) {
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={styles.container}>
        <View style={styles.header}>
          <View style={{ flex: 1 }}>
            <Text style={styles.title}>{mode === "advance" ? "Pay Secure Advance" : "Complete Payment"}</Text>
            {order && <Text style={styles.subtitle}>{order.advancePercentage ? `${order.advancePercentage}% advance` : "Remaining amount"} · ₹{order.amount.toLocaleString("en-IN")}</Text>}
          </View>
        ...
        {(loading || verifying) && <View style={styles.center}><ActivityIndicator size="large" color="#FF4D6D" /><Text style={styles.info}>{verifying ? "Verifying payment securely…" : "Preparing secure checkout…"}</Text></View>}
        {error && !loading && !verifying && <View style={styles.center}><MaterialIcons name="error-outline" size={44} color="#E63B5F" /><Text style={styles.error}>{error}</Text><TouchableOpacity style={styles.retry} onPress={prepare}><Text style={styles.retryText}>Try Again</Text></TouchableOpacity></View>}
        {order && !verifying && <WebView
...
const styles = StyleSheet.create({
  ...
  title: { color: "#3F1D2F", fontSize: 20, fontWeight: "800" }, subtitle: { color: "#8D6171", marginTop: 3 },
  close: { width: 40, height: 40, alignItems: "center", justifyContent: "center" },
  center: { flex: 1, alignItems: "center", justifyContent: "center", padding: 28 }, info: { color: "#6C2D45", marginTop: 14 }, error: { color: "#E63B5F", textAlign: "center", marginTop: 12 },
  retry: { marginTop: 18, backgroundColor: "#FF4D6D", borderRadius: 14, paddingHorizontal: 20, paddingVertical: 12 }, retryText: { color: "white", fontWeight: "700" },
});
